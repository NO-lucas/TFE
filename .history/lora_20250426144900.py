...
def compute_entropy(probs):
    """Calcul de l'entropie d'un vecteur de probabilités"""
...
def pipeline(args, clip_model, output_path):
    ...
    for slide_path in ndpi_paths:
        ...
        print("nb patches width x height : ", width_low_res//224, " x ", height_low_res//224)

        print("HIGHER RES nb patches width x height : ", slide.level_dimensions[0][0]//224, slide.level_dimensions[0][1]//224)
        ...
        data_for_features = []  # Stocke ici toutes les infos pour le reformattage

        ### DÉBUT Timer patch extraction
        t0 = time.time()

        print("Prediction on high resolution patches...")
        with torch.no_grad():
            for x in tqdm(range(0, width_low_res, patch_size), desc=f"High res X - {slide_name}"):
                for y in range(0, height_low_res, patch_size):
                    if is_background_patch(x, y, bg_mask, res_ratio, patch_size):
                        continue
                    tile = slide.read_region((int(x * low_scale_factor), int(y * low_scale_factor)), low_res, (224, 224)).convert("RGB")
        ...
        t1 = time.time()
        print(f"⏱️ Temps de parcours de la slide : {t1 - t0:.2f} secondes")

        ### DÉBUT Timer reformatage + prédiction SVM
        t2 = time.time()
        ...
        for idx, row in df_patches.iterrows():
            img_id = slide_name  # ici, une seule slide = un seul id
            if img_id not in features_dict:
                features_dict[img_id] = np.zeros((9, 2))  # 9 classes
            features_dict[img_id][row["prediction"], 0] += 1  # cell_count : nombre de fois qu'une classe est prédite
            features_dict[img_id][row["prediction"], 1] += np.mean(row["probabilities"])
        ...
        t3 = time.time()
        print(f"Reformatting time + prediction : {t3 - t2:.2f} seconds")
